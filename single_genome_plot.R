...
single_genome_plot <- function(input, output, data.chr, data.track, track_indx, chr_plotype, plot_direct, plot_type, layer_index,
                               Height, Width, col_type, color_cus, color_mulgp, col_transpt, height_layer, margin_layer, theme_sty, font_size, xtitle, 
                               ytitle, title_font_face, xlabel, ylabel, lgd_pos, lgd_space_size, lgd_intra_size, lgd_title_size, lgd_title_font_face, 
                               lgd_text_size, lgd_text_font_face, add_border, border_col, rect_col, rect_col_dis, rect_col_dis_cus, rect_grad_col, 
                               col_rect, sel_heatmap_col, hmap_col, heatmap_col_cus_2cols, heatmap_col_cus_3cols, rect_grad_cus_2cols, rect_grad_cus_3cols, 
                               hmap_col_dis, hmap_col_dis_cus, symbol_point, sel_symbol_point, point_size, sel_point_size, line_color, line_size, fill_area, 
                               sel_area_type, border_area, linetype, add_arrow, arrow_pos, arrow_size, text_col, text_size, font_face, text_angle, col_lgd, 
                               col_lgd_name, size_lgd, size_lgd_name, shape_lgd, shape_lgd_name, line_type_lgd, line_type_lgd_name, col_lgd_mdy_label, col_lgd_label, 
    ...
    for (i in indx) {
      ...
      if (plot_type[i] == "rect_discrete") {
        ...
        output$errorinfo3 <- renderPrint({
          if (input[[paste("plot_type", i, sep = "")]] == "rect_discrete" && input[[paste("rect_col", i, sep = "")]] != 1) {
            validate(need(
              all(areColors(jd_col)),
              paste(
                "Error: Data color error for Data", i, "!",
      ...
      if (plot_type[i] == "point") {
        ...
        output$errorinfo7 <- renderPrint({
          if (input[[paste("plot_type", i, sep = "")]] == "point") {
            validate(need(
